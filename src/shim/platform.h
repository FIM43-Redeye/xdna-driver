...
protected:
  ...
  // Exposed to subclasses: the emulator's platform_drv_emu replaces the
  // real DRM device fd with a memfd_create() fd, and the non-virtual
  // drv_mmap() reads m_dev_fd directly -- so there is no virtual seam,
  // the subclass must set the field.
protected:
